...
export function toReaderHtml(bodyHtml, bodyText, { fromName, date } = {}) {
  if (bodyHtml) {
    try {
      ...
      return `<!DOCTYPE html><html><head><base target="_blank"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="referrer" content="no-referrer"><style>${SHELL_CSS}</style>${emailStyles}</head><body>${header}${bodyContent}</body></html>`
    } catch (err) {
  ...
  return `<!DOCTYPE html><html><head><base target="_blank"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="referrer" content="no-referrer"><style>${SHELL_CSS} body{font-family:system-ui,sans-serif;} pre{white-space:pre-wrap;font-family:inherit;}</style></head><body>${header}<pre>${escaped}</pre></body></html>`
}
